    ...
    size: {
      ...
      validator: function validator(val) {
        if (typeof val === 'string') {
          return ['large', 'medium', 'small', 'extra-extra-small', 'extra-small', 'extra-large', 'default'].includes(val);
        }
...
// CONCATENATED MODULE: ./packages/avatar/src/group.js
...
/* harmony default export */ var group = ({
	...
	components: {
		Avatar: main["a" /* default */]
	},
	mounted: function mounted() {
		var items = this.$el.querySelectorAll(".el-avatar");
		if (items.length) {
			items.forEach(function (item, index) {
				item.style.setProperty("position", "relative");
				item.style.setProperty("z-index", 99 - index);
				item.style.setProperty("border", "2px solid #fff");
			});
		}
	},
	render: function render() {
		...
		if (typeof maxCount === "number" && this.$slots.default && this.$slots.default.length > maxCount) {
			var shouldRenders = this.$slots.default.slice(0, maxCount);
			var extraNumber = this.$slots.default.length - maxCount;
			var renderMoreSlot = this.$slots["more"];
			return h(
				"div",
				{ "class": "el-avatar-group" },
				[shouldRenders, renderMoreSlot || h("el-avatar", [h(
					"span",
...
// CONCATENATED MODULE: ./packages/avatar-group/index.js
...
/* harmony default export */ var avatar_group = __webpack_exports__["default"] = (group);
