	...
	expand: {
		...
		renderCell: function renderCell(h, _ref6) {
			...
			return h(
				...
				[h("i", { "class": "el-icon el-icon-caret-right" })]
			);
		},
		sortable: false,
		resizable: false,
...
function treeCellPrefix(h, _ref8, isTree) {
	var row = _ref8.row,
	...
	if (!treeNode) {
		var expandClasses = ["el-table__expand-icon"];
		var iconClasses = ["el-icon-caret-right"];
		return [h(
			"div",
			{
				"class": expandClasses,
				style: "opacity: 0;display:inline-block;width:20px;margin-right:8px;"
			},
	...
	if (typeof treeNode.expanded === "boolean" && !treeNode.noLazyChildren) {
		var _expandClasses = ["el-table__expand-icon", treeNode.expanded ? "el-table__expand-icon--expanded" : ""];
		var _iconClasses = ["el-icon-caret-right"];
		if (treeNode.loading) {
			_iconClasses = ["el-icon-loading"];
		}
		ele.push(h(
			"div",
			{ "class": _expandClasses, on: {
					"click": callback
				}
			},
			[h("i", { "class": _iconClasses })]
		));
	} else {
		...
		setColumnRenders: function setColumnRenders(column) {
			...
			if (column.type === "expand") {
				...
				column.renderCell = function (h, data) {
					...
					var isTree = _this2.$parent.tableData ? _this2.$parent.tableData.some(function (item) {
						return item[_this2.$parent.treeProps.children || "children"];
					}) : false;
					var prefix = treeCellPrefix(h, data, isTree);
					var props = {
						class: "cell",
						style: {}
					};
